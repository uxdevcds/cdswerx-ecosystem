...
class Admin_Config extends Rest_Base {
	...
	public function get_resources( array $config ) {
		$config['resourcesData'] = [
			...
			'resources' => [
				[
					'title'  => __( 'Help Center', 'cdswerx-theme' ),
					'link'   => ' https://go.elementor.com/hello-help/',
					'icon'   => 'HelpIcon',
					'target' => '_blank',
				],
				[
					'title'  => __( 'Blog', 'cdswerx-theme' ),
					'link'   => 'https://go.elementor.com/hello-blog/',
					'icon'   => 'SpeakerphoneIcon',
					'target' => '_blank',
				],
				[
					'title'  => __( 'Platinum Support', 'cdswerx-theme' ),
	...
	public function get_site_parts( array $config ): array {
		...
		$general = [
			[
				'title' => __( 'Add New Page', 'cdswerx-theme' ),
				'link'  => self_admin_url( 'post-new.php?post_type=page' ),
				'icon'  => 'PageTypeIcon',
			],
			[
				'title' => __( 'Settings', 'cdswerx-theme' ),
				'link'  => self_admin_url( 'admin.php?page=cdswerx-theme-settings' ),
			],
		];
		...
		if ( Utils::is_elementor_active() ) {
			$common_parts = [
				[
					'title' => __( 'Theme Builder', 'cdswerx-theme' ),
	...
	private function update_pro_part( array $part, string $location ): array {
		...
		$part['sublinks'] = [
			[
				'title' => __( 'Edit', 'cdswerx-theme' ),
				'link'  => $edit_link,
			],
			[
				'title' => __( 'Add New', 'cdswerx-theme' ),
	...
	public function get_quicklinks( $config ): array {
		$config['quickLinks'] = [
			'site_name'    => [
				'title' => __( 'Site Name', 'cdswerx-theme' ),
				'link'  => $this->get_open_homepage_with_tab( 'settings-site-identity', [ 'autofocus[section]' => 'title_tagline' ] ),
				'icon'  => 'TextIcon',

			],
			'site_logo'    => [
				'title' => __( 'Site Logo', 'cdswerx-theme' ),
				'link'  => $this->get_open_homepage_with_tab( 'settings-site-identity', [ 'autofocus[section]' => 'title_tagline' ] ),
				'icon'  => 'PhotoIcon',
			],
			'site_favicon' => [
				'title' => __( 'Site Favicon', 'cdswerx-theme' ),
				'link'  => $this->get_open_homepage_with_tab( 'settings-site-identity', [ 'autofocus[section]' => 'title_tagline' ] ),
				'icon'  => 'AppsIcon',
			],
		];
	...
	public function get_welcome_box_config( array $config ): array {
		...
		if ( ! $is_elementor_active ) {
			...
			if ( 'activate-elementor' === $action_link_type ) {
				$cta_text = __( 'Activate Elementor', 'cdswerx-theme' );
			} else {
				$cta_text = __( 'Install Elementor', 'cdswerx-theme' );
			}

			$config['welcome'] = [
				'title'   => __( 'Thanks for installing the Hello Theme!', 'cdswerx-theme' ),
				'text'    => __( 'Welcome to Hello Theme—a lightweight, blank canvas designed to integrate seamlessly with Elementor, the most popular, no-code visual website builder. By installing and activating Elementor, you\'ll unlock the power to craft a professional website with advanced features and functionalities.', 'cdswerx-theme' ),
				'buttons' => [
					[
						'linkText' => $cta_text,
						'variant'  => 'contained',
				...
				'image'   => [
					'src' => CDSWERX_THEME_IMAGES_URL . 'install-elementor.png',
					'alt' => $cta_text,
				],
			];
		...
		if ( $is_elementor_active && ! $has_pro ) {
			$config['welcome'] = [
				'title'   => __( 'Go Pro, Go Limitless', 'cdswerx-theme' ),
				'text'    => __( 'Unlock the theme builder, popup builder, 100+ widgets and more advanced tools to take your website to the next level.', 'cdswerx-theme' ),
				'buttons' => [
					[
						'linkText' => __( 'Upgrade now', 'cdswerx-theme' ),
						'variant'  => 'contained',
